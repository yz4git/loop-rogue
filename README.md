...
主人公は7×7盤面の中央に固定され、プレイヤーが行または列を1マスずつ
スライドさせて世界を動かす、iPhone Safari向けローグライトパズルです。

## 遊び方

- 盤面の任意の行を左右へスワイプ、または列を上下へスワイプします。
- 指を離すと最寄りの1マスに吸着し、それが1ターンになります。
- 端から出たオブジェクトは反対側から現れます。
- 鍵を中央へ運んで取得し、その後に出口を中央へ運ぶとフロアクリアです。
- トゲはダメージ、回復薬は回復、スライムは中央へ運ぶと自動攻撃します。
- 中央に隣接する岩を主人公へ押し込む方向は、岩に遮られて動かせません。
...
フロア生成時には鍵から出口までの手順を組み立て、罠と敵フェーズを含めて
攻略可能性を検証してから盤面を採用します。
